"""
gui/emu_display.py
EmuDisplayMixin - EMU 模式欄位變更高亮與設定顯示 mixin
"""
...
class EmuDisplayMixin:
    """EMU 模式設定顯示 mixin：變更高亮、原值標注、設定目前狀態顯示"""
    ...
    def update_current_config_display(self):
        ...
        original = self._original_config or {}
        ...
        for key, spinner in self.threshold_spinners.items():
            orig_val = float(original.get("thresholds_emu", {}).get(key, spinner.value()))
            changed = abs(spinner.value() - orig_val) > 1e-9
            self._set_widget_changed_style(spinner, changed)
        ...
        for serial, check in self.device_energy_checks.items():
            orig_val = bool(original.get("device_strategies", {}).get(serial, original.get("energy_strategy", False)))
            self._set_checkbox_changed_text(check, check.isChecked() != orig_val, self._fmt_original_bool(orig_val))
        ...
        reconnect_fields = [
            ...
            (self.reconnect_wait_right_form, self.check_game_open_interval_spin, float(original_disconnect.get("check_game_open_interval_emu", self.check_game_open_interval_spin.value())), 0),
        ...
        orig_diff = float(original_disconnect.get("screen_hash_diff_threshold", self.screen_hash_diff_threshold_spin.value()))
        diff_changed = abs(float(self.screen_hash_diff_threshold_spin.value()) - orig_diff) > 1e-9
        self._set_widget_changed_style(self.screen_hash_diff_threshold_spin, diff_changed)
        if hasattr(self, "screen_hash_diff_label"):
            self._set_label_changed_text(self.screen_hash_diff_label, diff_changed, self._fmt_original_number(orig_diff, 1))
        ...
        attempts_label = self.max_reconnect_label if hasattr(self, "max_reconnect_label") else None
        if attempts_label:
            self._set_label_changed_text(attempts_label, attempts_changed, "∞" if orig_unlimited else str(orig_attempts_display))
        ...
        orig_pkg = str(original_disconnect.get("emu_package_name", "") or "")
        pkg_changed = self.emu_package_input.text().strip() != orig_pkg
        self._set_widget_changed_style(self.emu_package_input, pkg_changed)
        if hasattr(self, "emu_package_label"):
            self._set_label_changed_text(self.emu_package_label, pkg_changed, orig_pkg or t("config_not_set", "未設定"))
        ...
        for serial, widgets in self.device_auto_feature_checks.items():
            saved = original.get("device_auto_features", {}).get(serial, {})
            orig_wander = bool(saved.get("wander", original_disconnect.get("auto_enable_wander", True)))
            orig_ai = bool(saved.get("ai", original_disconnect.get("auto_enable_ai", True)))
            self._set_checkbox_changed_text(widgets["wander"], widgets["wander"].isChecked() != orig_wander, self._fmt_original_bool(orig_wander))
            self._set_checkbox_changed_text(widgets["ai"], widgets["ai"].isChecked() != orig_ai, self._fmt_original_bool(orig_ai))
